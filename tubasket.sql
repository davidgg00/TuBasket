...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-06-2020 a las 18:42:21
...
CREATE TABLE `equipos` (
  ...
  `liga` varchar(25) COLLATE utf8_spanish_ci NOT NULL,
...
INSERT INTO `equipos` (`id`, `equipo`, `pabellon`, `ciudad`, `escudo_ruta`, `liga`, `partidos_ganados`, `partidos_perdidos`, `puntos_favor`, `puntos_contra`, `puntos_clasificacion`) VALUES
...
CREATE TABLE `fichajes` (
  `id` int(11) NOT NULL,
  `EntrenadorSolicita` varchar(25) COLLATE utf8_spanish_ci NOT NULL,
  `username_jugador1` varchar(25) COLLATE utf8_spanish_ci NOT NULL,
  `EntrenadorRecibe` varchar(25) COLLATE utf8_spanish_ci NOT NULL,
  `username_jugador2` varchar(25) COLLATE utf8_spanish_ci NOT NULL,
...
UPDATE usuarios SET equipos = (SELECT equipos FROM usuarios WHERE username = old.EntrenadorSolicita) WHERE username = old.username_jugador1;

UPDATE usuarios SET equipos = (SELECT equipos FROM usuarios WHERE username = old.EntrenadorRecibe) WHERE username = old.username_jugador2;
END IF;
END
$$
DELIMITER ;
...
CREATE TABLE `jugador_stats` (
  ...
  `jugador` varchar(25) COLLATE utf8_spanish_ci NOT NULL,
  `triples_metidos` int(3) NOT NULL,
  `tiros_2_metidos` int(3) NOT NULL,
  `tiros_libres_metidos` int(3) NOT NULL,
  `tapones` int(3) NOT NULL,
  `robos` int(3) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_spanish_ci;
...
CREATE TABLE `ligas` (
  ...
  `administrador` varchar(25) COLLATE utf8_spanish_ci NOT NULL,
...
INSERT INTO `ligas` (`nombre`, `password`, `administrador`, `ganador`) VALUES
...
INSERT INTO `partidos` (`id`, `local`, `visitante`, `resultado_local`, `resultado_visitante`, `fecha`, `hora`, `jornada`, `liga`) VALUES
...
--
DELIMITER $$
CREATE TRIGGER `puntos_clasificacion` AFTER UPDATE ON `partidos` FOR EACH ROW BEGIN
...
CREATE TABLE `usuarios` (
  ...
  `email` varchar(50) COLLATE utf8_spanish_ci NOT NULL,
  `apenom` varchar(35) COLLATE utf8_spanish_ci NOT NULL,
  `fecha_nac` date NOT NULL,
  `liga` varchar(25) COLLATE utf8_spanish_ci DEFAULT NULL,
...
('admin', 'ba3253876aed6bc22d4a6ff53d8406c6ad864195ed144ab5c87621b6c233b548baeae6956df346ec8c17f5ea10f35ee3cbc514797ed7ddd3145464e2a0bab413', 'Administrador', 'admin@admin.com', 'admin admin', '2000-12-12', NULL, NULL, 1, 'assets/uploads/perfiles/pordefecto.png'),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_jugadores_partidos`  AS  select `p`.`id` AS `idpartido`,`u`.`apenom` AS `apenom`,`u`.`username` AS `username`,`u`.`equipo` AS `idequipo`,`e`.`equipo` AS `equipo`,`e`.`escudo_ruta` AS `escudo_ruta_local` from ((`equipos` `e` join `usuarios` `u` on(`e`.`id` = `u`.`equipo`)) join `partidos` `p` on(`e`.`id` = `p`.`local` or `e`.`id` = `p`.`visitante`)) where `u`.`tipo` like 'Jugador' and `u`.`validado` like '1' order by `p`.`id` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_usuarios_liga`  AS  select `u`.`username` AS `username`,`u`.`password` AS `password`,`u`.`tipo` AS `tipo`,`u`.`email` AS `email`,`u`.`apenom` AS `apenom`,`u`.`fecha_nac` AS `fecha_nac`,`u`.`liga` AS `liga`,`u`.`equipo` AS `equipo`,`u`.`imagen` AS `foto_perfil`,`u`.`validado` AS `validado`,`e`.`equipo` AS `nombre_equipo` from (`usuarios` `u` join `equipos` `e` on(`u`.`equipo` = `e`.`id`)) ;
...
--
ALTER TABLE `equipos`
  ADD CONSTRAINT `equipo_ibfk_1` FOREIGN KEY (`liga`) REFERENCES `ligas` (`nombre`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `jugador_stats`
  ...
  ADD CONSTRAINT `jugador_stats_ibfk_2` FOREIGN KEY (`id_partido`) REFERENCES `partidos` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `ligas`
  ...
  ADD CONSTRAINT `liga_ibfk_2` FOREIGN KEY (`ganador`) REFERENCES `equipos` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `partidos`
  ADD CONSTRAINT `partido_ibfk_1` FOREIGN KEY (`local`) REFERENCES `equipos` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `partido_ibfk_2` FOREIGN KEY (`visitante`) REFERENCES `equipos` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `partido_ibfk_3` FOREIGN KEY (`liga`) REFERENCES `ligas` (`nombre`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `usuarios`
  ADD CONSTRAINT `usuarios_ibfk_1` FOREIGN KEY (`liga`) REFERENCES `ligas` (`nombre`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
